...
def search_uploader_for_med(fb_id,medicine_name,data2):
    mysql = med()
    db = mysql.connect()
    cursor = db.cursor()
    cursor.execute("""SELECT u.cust_id,u.uname,u.location,u.phoneno,u.email,a.qty
                    from users u,med_acc a
                    where a.qty>=%d u.cust_id=a.cust_id and u.cust_id in(select cust_id
		 			                                           from med_acc
		 			                                            where med_id in(select med_id
		                                                    		 			   from med_det
				 			                                                        where trade_name=%s))""", (data2,medicine_name))
